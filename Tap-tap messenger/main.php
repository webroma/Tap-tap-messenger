...
?>
            ...
            <?php
// Инициализация переменной для хранения информации
$user_info = "";

// Проверка, были ли отправлены данные из формы
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['user_info'])) {
    // Получаем информацию о пользователе и очищаем её от потенциально опасных символов
    $user_info = htmlspecialchars(trim($_POST['user_info']));
}
?>
    <div class="inf">
        <p id="aboutme">Информация о себе: <?php echo $user_info; ?></p>
    </div>
        </div> 
        </div>
    ...
    <section class="main">
        ...
        <div class="findcont">
            <p class="netcont">На данный момент нет доступных контактов.<br>Не переживайте!<br> Используйте поиск, чтобы добавить нужные контакты.</p>
        </div>
        <div class="inp-msg">
